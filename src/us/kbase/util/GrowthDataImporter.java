...
public class GrowthDataImporter {
	...
	public List<String> generateGrowthMatrix(String name) throws TokenFormatException, IOException, JsonClientException {
		List<String> result = new ArrayList<String>();
		GrowthMatrix matrix = new GrowthMatrix();
		matrix.setName(name);
		matrix.setDescription("FW510-R11 in R2A, Nickel");
		...
		ConcentrationSeriesMetadata concentrationSeriesMetadata = new ConcentrationSeriesMetadata().withBaseMedia("Unspecified").withSeriesElemCompound("Nickel").withUnit("microM");
			...
			while ((line = br.readLine()) != null) {
				if (line.equals("")) {
					...
					for (int i = 1; i < fields.length; i++) { 
						...
						concentrationValues.add(Double.valueOf(fields[i]));
...
*/
		...
		concentrationSeriesMetadata.setConcentrationValues(concentrationValues);
		...
		GenericSeriesMetadata genericSeriesMetadata = new GenericSeriesMetadata().withSeriesMetadataType("ConcentrationSeriesMetadata").withElemLabels(sampleNames).withSeriesMetadata(UObject.transformObjectToObject(concentrationSeriesMetadata, UObject.class));
		
		matrix.setColumnMetadata(timeSeriesMetadata);
		matrix.setRowMetadata(genericSeriesMetadata);
		
		//System.out.println(matrix.toString());
		WsDeluxeUtil.saveObjectToWorkspaceRef(UObject.transformObjectToObject(matrix, UObject.class), "KBaseEnigmaMetals.GrowthMatrix", workspaceId, name, "https://ci.kbase.us/services/ws", token.toString());
